...
//========================================================================================================================================================
const news = document.querySelector('.news__header');
news.addEventListener("click", function (e) {
	const targetElem = e.target;
	if (targetElem.classList.contains('news__all')) {
		if (!targetElem.classList.contains('_loading')) {
			targetElem.classList.add('_loading');
			getMoreAlls(targetElem);
		}
		e.preventDefault();
	}

});
async function getMoreAlls(targetElem) {

	const response = await fetch('data/faq_news.html');
	if (response.ok) {
		const text = await response.text();
		const moreAlls = document.querySelector('.news__items');
		moreAlls.insertAdjacentHTML('beforeend', text);
		targetElem.classList.remove('_loading');
	}
	else {
		alert('Что-то пошло не так...');
...
let allCurrentSlides = document.querySelector('.our-spaces ._fraction__current');
...
sliderOurSpaces = new Swiper('.slider-our-spaces__body', {
	...
	// Dotts
	navigation: {
		nextEl: '._slider-arrow_right',
		prevEl: '._slider-arrow_left',
	},
	pagination: {
	...
	on: {
		...
		slideChange: function (slider) {
			...
			allCurrentSlides.innerHTML = setZeroFormat(currentSlide);
		},
	}
...
sliderOurSpaces.on('slideChange',)
//========================================================================================================================================================
...
function initOurFacilitiesSlider() {
	...
	if (sliderBlocks.length > 0) {
		sliderBlocks.forEach((sliderBlock, index) => {
			...
			let allCurrentOurFacilitiesSliders = document.querySelectorAll('.controls-our-facilities__item')[index].querySelector('._fraction__current');
			...
			const sliderItem = new Swiper(slider, {
				...
				on: {
					...
					slideChange: function (slider) {
						let currentSlide = ++slider.realIndex;
						allCurrentOurFacilitiesSliders.innerHTML = setZeroFormat(currentSlide);
					},
				}
			});
...
function inputs_init(inputs) {
	if (inputs.length > 0) {
		for (let index = 0; index < inputs.length; index++) {
			...
			if (input.classList.contains('_date')) { //["Sun", "Tue", "Wed", "Thu", "Fri", "Sat", "Mon"]
